...
let rec step = function
  ...
  | S.Pair (v1, v2) as v when is_value v -> failwith "Expected a non-terminal expression"
  ...
  | S.Snd (S.Pair (v1, e2)) when is_value v1 ->  S.Fst (S.Pair (v1, step e2))
  | S.Snd (S.Pair (e1, e2)) -> S.Snd (S.Pair (step e1, e2))
  | S.Snd e -> S.Snd (step e)
  | S.Nil -> failwith "Expected a non-terminal expression"
  | S.Cons (v1, v2) as v when is_value v -> failwith "Expected a non-terminal expression"
